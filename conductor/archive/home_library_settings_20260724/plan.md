...
- [x] Task: Build Compose `HomeScreen` with TopAppBar, Search, and Tabbed Dashboard (Songs, Albums, Artists).
- [x] Task: Build `MiniPlayer` persistent bottom bar anchored above navigation bar.
- [x] Task: Connect track selection from `HomeScreen` to `PlayerManager` playback and mini-player expansion.
- [x] Task: Phase Verification & Checkpoint (Refer to workflow.md)

## Phase 2.3: Comprehensive Settings Screen
- [x] Task: Build Compose `SettingsScreen` with theme toggle, ReplayGain adjustment, manual scanner trigger, and About section.
- [x] Task: Integrate Settings navigation and Preferences DataStore.
- [x] Task: Phase Verification & Checkpoint (Refer to workflow.md)
